...
class GerenciadorMemoriaFragmentacaoInterna:
    ...
    def mostrar_memoria(self):
        ...
        print("="*85)
        print(f"Tamanho total da memória: {self.tamanho_total} bytes")
        print(f"Tamanho da página: {self.tamanho_pagina} bytes")
        print("-" * 85)
        print(f"{'Endereço':<15} {'Tamanho':<10} {'Status':<25} {'Uso (bytes)':<15} {'Frag. Interna':<15}")
        print("-" * 85)

        for bloco in self.memoria:
            inicio_fim = f"[{bloco.inicio:4d}-{bloco.fim:4d}]"
            tamanho = f"{bloco.tamanho:4d}"
            if bloco.processo:
                status = f"Processo {bloco.processo}"
                uso = f"{bloco.tamanho_processo:>5d}"
                frag = f"{bloco.fragmentacao_interna:>5d}"
            else:
                status = "LIVRE"
                uso = "-"
                frag = "-"
            print(f"{inicio_fim:<15} {tamanho:<10} {status:<25} {uso:<15} {frag:<15}")
    ...
    def _mostrar_estatisticas(self):
        ...
        print(f"Fragmentação interna total acumulada: {self.estatisticas['fragmentacao_interna_total']} bytes")
        print(f"Memória total perdida: {self.estatisticas['memoria_perdida_total']} bytes")
